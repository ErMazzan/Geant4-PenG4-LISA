...
void PenInterface::PEMATC(char matnam[62], int nele, int iatmn[30], double frmas[30], 
            double dnst, FILE* IWR)
{
 
  //  This subroutine generates the material definition file, a part of the
  //  input data file of PENELOPE.
  //
  //  matnam : material name (max 62 characters)
  //  nele   : number of elements (max 30)
  //  iatmn  : atomic number of n-th element
  //  frmas  : fraction mass of n-th element
  //  dnst   : density (g/cm3)
  //  IWR    : output file
...
//MA    printf("Enter material name, for your information (no more than 60 characters) ...\n");
//MA    scanf("%62s", NAME);
//MA    printf(" Material: %s\n", NAME);
    strcpy(NAME,matnam);
    NAME[62]='\0';
...
//MA    {
//MA      brkIt = true;
//MA      printf(" Chemical formula:\n");
//MA      printf(" Number of elements in the molecule...\n");
//MA      scanf("%d", &NELEM[M]);
//MA      if(NELEM[M] < 1 || NELEM[M] > 30)
//MA      {
//MA        printf(" NELEM must be positive and less than 31.\n");
//MA        printf(" Please, enter an allowed value.\n");
//MA        brkIt = false;
//MA      }
//MA    }
//MA    printf(" Number of elements = %2d\n", NELEM[M]);
    NELEM[M] = nele;
...
//MA        if(IZZ < 1 || IZZ > 99)
//MA        {
//MA          printf(" The atomic number must be in the range 1-99.\n");
//MA          printf(" Please, enter an allowed value.\n");
//MA          brkIt = false;
//MA        }
//MA      }
//MA      IZ[M][0] = IZZ;
//MA      STF[M][0] = 1.0;
//MA      printf("  Element: %2s (Z=%2d), atoms/molecule = %12.5E\n", LASYMB[IZZ-1], IZZ, STF[M][0]);
//MA      ZT[M] = IZZ;
//MA      AT[M] = ATW[IZZ-1];
//MA      EXPOT[M] = IZZ*log(EPX[IZZ-1]);
//MA    }
//MA    else
//MA    {
//MA      printf(" Select one option (1 or 2):\n");
//MA      printf("   1: Enter chemical (stoichiometric) formula\n");
//MA      printf("   2: Enter fraction by weight of each element\n");
//MA      scanf("%d", &IREAD2);
//MA    
//MA      if(IREAD2 == 2)
//MA      {
...
//MA            if(I == 0)
//MA            {
//MA              printf("  Enter atomic number and fraction by weight of the first element ...\n");
//MA            }
//MA            else if(I == 1)
//MA            {
//MA              printf("  Enter atomic number and fraction by weight of the second element ...\n");
//MA            }
//MA            else if(I == 2)
//MA            {
//MA              printf("  Enter atomic number and fraction by weight of the third element ...\n");
//MA            }
//MA            else
//MA            {
//MA              printf("  Enter atomic number and fraction by weight of the %d-th element ...\n", I+1);
//MA            }
//MA            scanf("%d %lf", &IZZ, &FBW[I]);
            IZZ = iatmn[I];
            FBW[I] = frmas[I];
...
//MA            if(FBW[I] <= 0.0)
//MA            {
//MA              printf(" The fraction by weight must be positive.\n");
//MA              printf(" Please, enter a positive value.\n");
//MA              brkIt = false;
//MA            }
//MA          }
//MA          printf("\n");
          IZ[M][I] = IZZ;
          printf("  Element: %2s (Z=%2d), fraction by weight = %12.5E\n", LASYMB[IZZ-1], IZZ, FBW[I]);
          if(I > 0)
          {
            for(int K = 0; K <= I-1; K++)
            {       
              if(IZZ == IZ[M][K]){ ErrorFunction(1101); return;}
            }
          }
        ...
        if(STFM < 1.0E-16){ ErrorFunction(1102); return;};
        for(int I = 0; I < NELEM[M]; I++)
        {
...
//MA      {
//MA
//MA        printf(" Stoichiometric indexes...\n");
//MA        for(int I = 0; I < NELEM[M]; I++)
//MA        {
//MA          brkIt = false;
//MA          while(!brkIt)
//MA          {
...
//MA            if(I == 0)
//MA            {
//MA              printf("  Enter atomic number and number of atoms/molecule of the first element ...\n");
//MA            }
//MA            else if(I == 1)
//MA            {
//MA              printf("  Enter atomic number and number of atoms/molecule of the second element ...\n");
//MA            }
//MA            else if(I == 2)
//MA            {
//MA              printf("  Enter atomic number and number of atoms/molecule of the third element ...\n");
//MA            }
//MA            else
//MA            {
//MA              printf("  Enter atomic number and number of atoms/molecule of the %d-th element ...", I+1);
//MA            }
//MA            scanf("%d %lf", &IZZ, &STF[M][I]);   
//MA            if(IZZ < 1 || IZZ > 99)
//MA            {
//MA              printf(" The atomic number must be in the range 1-99.\n");
//MA              printf(" Please, enter an allowed value.\n");
...
//MA            if(STF[M][I] <= 0.0)
//MA            {
//MA              printf(" The stoichiometric fraction must be positive.\n");
//MA              printf(" Please, enter a positive value.\n");
//MA              brkIt = false;
//MA            }
//MA          }
//MA          IZ[M][I] = IZZ;
//MA          printf("  Element %s (Z=%2d), atoms/molecule = %12.5E\n", LASYMB[IZZ-1], IZZ, STF[M][I]);
//MA          if(I > 0)
//MA          {
//MA            for(int K = 0; K <= I-1; K++)
//MA            {       
//MA              if(IZZ == IZ[M][K]){ ErrorFunction(1103); return;}
//MA            }         
//MA          }
//MA          ZT[M] = ZT[M]+IZZ*STF[M][I];
//MA          AT[M] = AT[M]+ATW[IZZ-1]*STF[M][I];
//MA          EXPOT[M] = EXPOT[M]+IZZ*log(EPX[IZZ-1])*STF[M][I];
//MA        }
//MA      }
//MA    }
...
//MA    printf(" Do you want to change it?   (1=yes,2=no)\n");
//MA    int IYESNO;
//MA    scanf("%d", &IYESNO);
//MA    if(IYESNO == 1)
//MA    {
...
//MA        scanf("%lf", &EXPOT[M]);
//MA        printf("  Mean excitation energy =%12.5E eV\n", EXPOT[M]);
//MA        if(EXPOT[M] < 1.0)
//MA        {
//MA          printf(" The mean exc. energy must be larger than 1 eV.\n");
//MA          printf(" Please, enter a valid value.\n");
//MA          brkIt = false;
//MA        }
//MA      }
//MA    }
...
//MA  else
//MA  {
//MA ++++++++++++++++ Lines in else-block are deleted
//MA  }
