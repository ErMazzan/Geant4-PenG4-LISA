...
//                       SUBROUTINE ESIaR
//  *********************************************************************
void PenInterface::ESIaR(int M, FILE* IRD, FILE* IWR, int INFO)
{
...
/////////////  using namespace CEGRID;
  ...
  for(int IEL = 0; IEL < NELEM[M-1]; IEL++)
  {
    ...
    if(IZZ != IZ[M-1][IEL]){ ErrorFunction(1304); return;}
    if(NDATA > NDIN){ ErrorFunction(1305); return;}
    if(NSHR > 16){ ErrorFunction(1306); return;}
        ...
        if(N > 3)
        {
          for(int I = 0; I < NEGP; I++)
          {
            ...
            { 
              ...
              if(DX > 1.0E-6)
              {
                XESI[IC-1][IS] = Y[J-1]+(XC-X[J-1])*(Y[J+1-1]-Y[J-1])/DX;
              }
              else
              {
                XESI[IC-1][IS] = (Y[J+1-1]+Y[J-1])/2.0;
              }
            }
            else
            {
