...
//                       SUBROUTINE PEMATW
//  *********************************************************************
void  PenInterface::PEMATW(int IFILE, char MFNAME[20])
{
...
/////////////  using namespace CEGRID;   // This is a struct
  ...
  if(IREAD == 1)
  {
    ...
    printf("Enter material name, for your information (no more than 60 characters) ...\n");
    scanf("%62s", NAME);
    printf(" Material: %s\n", NAME);
      ...
      if(IREAD2 == 2)
      {
        ...
        for(int I = 0; I < NELEM[M]; I++)
        {
          ...
          while(!brkIt)
          {
            ...
            if(I == 0)
            {
              printf("  Enter atomic number and fraction by weight of the first element ...\n");
            }
            else if(I == 1)
            {
              printf("  Enter atomic number and fraction by weight of the second element ...\n");
            }
            else if(I == 2)
            {
              printf("  Enter atomic number and fraction by weight of the third element ...\n");
            }
            else
            {
              printf("  Enter atomic number and fraction by weight of the %d-th element ...\n", I+1);
            }
      ...
      else //IREAD2=1
      {
        ...
        for(int I = 0; I < NELEM[M]; I++)
        {
          ...
          while(!brkIt)
          {
            ...
            if(I == 0)
            {
              printf("  Enter atomic number and number of atoms/molecule of the first element ...\n");
            }
            else if(I == 1)
            {
              printf("  Enter atomic number and number of atoms/molecule of the second element ...\n");
            }
            else if(I == 2)
            {
              printf("  Enter atomic number and number of atoms/molecule of the third element ...\n");
            }
            else
            {
              printf("  Enter atomic number and number of atoms/molecule of the %d-th element ...", I+1);
            }
          ...
          if(I > 0)
          {
            for(int K = 0; K <= I-1; K++)
            {       
              if(IZZ == IZ[M][K]){ ErrorFunction(1103); return;}
            }         
          }
          ZT[M] = ZT[M]+IZZ*STF[M][I];
          AT[M] = AT[M]+ATW[IZZ-1]*STF[M][I];
  ...
  {
    printf("\n");
    printf(" Enter the oscillator strength Fcb and energy Wcb (in eV) of the plasmon ...\n");
    scanf("%lf %lf", &FP, &EP);
    if(FP < 0.5)
    {
      FP = 0.0;
...
//////MA epmin and epmax are defined in PenInterface.hh
//MACG This must be forced to EMIN and EMAX, according to penelope source code
  CEGRID_.EMIN=50.0;
  double EMAX = 1.0e9;
